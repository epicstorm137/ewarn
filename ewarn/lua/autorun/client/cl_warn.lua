...
function ewarn.OpenWarns()
    ...
    warns:AddColumn("Name",3)
    warns:AddColumn("Reason",4)
    warns:AddColumn("Admin Name",5)
    warns:AddColumn("Admin SteamID",6)
    ...
    players.OnRowSelected = function( panel, index, row )
        ...
        net.Receive("ewarn_send",function()
            ...
            for k,v in ipairs(data) do
                local info = util.JSONToTable(v.data)
                local line = warns:AddLine(v.id, info.Date ,info.Name, info.Reason, info.AdminName, info.AdminID)
                line.OnSelect = function()
                    stat.Warn = "Reason: "..line:GetColumnText(4)
                end
                line.OnRightClick = function()
                    local rightclick = DermaMenu()
                    rightclick:AddOption("Copy Reason",function() SetClipboardText(line:GetColumnText(4)) end)
                    rightclick:AddOption("Copy Admin's SteamID",function() SetClipboardText(line:GetColumnText(6)) end)
                    ...
                    if ewarn.Config.AllowedRanks[LocalPlayer():GetUserGroup()] then
                        ...
                        rightclick:AddOption("Edit Warning",function()
                            Derma_StringRequest("EWarn - Edit","Enter the new reason:",info.Reason,function(text)
